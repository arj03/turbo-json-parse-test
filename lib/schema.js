...
function inferRawSchema (obj, opts, name) {
  let t = type(obj)
  if (name === 'anyOf') t = ANYOF
  else if (name == 'arbitraryJSON') t = ARBITRARYJSON
  ...
  if (t == ANYOF) {
    ...
    for (let i = 0; i < obj.length; ++i) {
      ...
      prop.fields.push(inferRawSchema(obj[i][key], opts, key))
    }
    prop.name = prop.fields[0].name
    return prop
  }
  else if (t == ARBITRARYJSON) {
    prop.name = Object.keys(obj)[0]
    return prop
  }
  else if (t === OBJECT) {
    prop.fields = []
