...
function defaults (opts) {
  ...
  function compileSbotContent (gen, prop, rawSchema, compileAny) {
    ...
    gen(`} else if (${ch('ptr+17')} === ${code('c')} && ${ch('ptr+18')} === ${code('h')} && ${ch('ptr+19')} === ${code('a')} && ${ch('ptr+20')} === ${code('n')} && ${ch('ptr+21')} === ${code('n')}  && ${ch('ptr+22')} === ${code('e')} && ${ch('ptr+23')} === ${code('l')} && ${ch('ptr+24')} === ${code('"')}) {`)
    ...
    gen(`} else if (${ch('ptr+17')} === ${code('c')} && ${ch('ptr+18')} === ${code('h')} && ${ch('ptr+19')} === ${code('e')} && ${ch('ptr+20')} === ${code('s')} && ${ch('ptr+21')} === ${code('s')} && ${ch('ptr+22')} === ${code('_')} && ${ch('ptr+23')} === ${code('m')} && ${ch('ptr+24')} === ${code('o')} && ${ch('ptr+25')} === ${code('v')} && ${ch('ptr+26')} === ${code('e')} && ${ch('ptr+27')} === ${code('"')}) {`)
    ...
    gen(`} else if (${ch('ptr+17')} === ${code('c')} && ${ch('ptr+18')} === ${code('h')} && ${ch('ptr+19')} === ${code('e')} && ${ch('ptr+20')} === ${code('s')} && ${ch('ptr+21')} === ${code('s')} && ${ch('ptr+22')} === ${code('_')} && ${ch('ptr+23')} === ${code('i')} && ${ch('ptr+24')} === ${code('v')} && ${ch('ptr+25')} === ${code('i')} && ${ch('ptr+26')} === ${code('t')} && ${ch('ptr+27')} === ${code('e')} && ${ch('ptr+28')} === ${code('"')}) {`)

    const chessInviteObj = {
      type: schema.OBJECT,
      required: false,
      defaults: false,
      name: prop.name,
      fields: [
        getField({ type: 'string' }, 'type'),
        getField({ type: 'string' }, 'inviting'),
        getField({ type: 'string' }, 'myColor'),
      ]
    }
    compileAny(gen, new Property(gen, prop.parent, chessInviteObj), chessInviteObj)

    gen(`} else if (${ch('ptr+17')} === ${code('t')} && ${ch('ptr+18')} === ${code('a')} && ${ch('ptr+19')} === ${code('g')}) {`)
